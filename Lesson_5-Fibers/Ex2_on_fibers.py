...
params = {"length": 80, "loss_coef": 0.2, "length_units": "km", "att_in": 0, "con_in": 0.5, "con_out": 0.5,
          "type_variety": "SSMF", "dispersion": 1.67e-5, "gamma": 0.00127}
...
# SNRs after the fiber
Rx_signal._calc_snr(si)
title = "Signal-to-Noise Ratios after fiber"
ml.plot_receiver(Rx_signal, si, title)

ml.plot_signal_NLI(si)  # signal power after the fiber
...
ml.plot_powers(si)  # signal power after the amp

# SNRs after the EDFA
Rx_signal._calc_snr(si)
title = "Signal-to-Noise Ratios after fiber and EDFA"
ml.plot_receiver(Rx_signal, si, title)
